...
const AnimeSection = ({ title, subtitle, children, showViewAll = false, viewAllText = "VER HISTORIAL", reducedTopPadding = false, onViewAllClick }: AnimeSectionProps) => {
  ...
  useEffect(() => {
    const updateImageHeight = () => {
      if (carouselRef.current) {
        const firstImage = carouselRef.current.querySelector('.aspect-video, .aspect-\\[2\\/3\\]');
        if (firstImage) {
  ...
  return (
    <section className={`${reducedTopPadding ? 'pt-0 max-[500px]:pt-0' : 'pt-6 max-[500px]:pt-1'} pb-6 max-[500px]:pb-1`}>
      <div className="flex items-end justify-between mb-1.5 px-[20px] xs:px-[30px] lg:px-[clamp(30px,4.95vw,78px)]">
        <div>
