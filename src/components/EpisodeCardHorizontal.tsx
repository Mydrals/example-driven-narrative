...
const EpisodeCardHorizontal = ({ 
  ...
  const handleMarkAsWatched = async (e?: React.MouseEvent) => {
    ...
    try {
      // Parse duration string (e.g., "24min" -> 1440 seconds)
      const durationInSeconds = parseInt(duration) * 60;

      // If already watched, unmark it (set progress to 0)
      const isWatched = progressPercentage >= 95;
      const newProgressTime = isWatched ? 0 : durationInSeconds;
  ...
  return (
    ...
    >
      <div className="flex gap-3 items-start">
        <div 
          className="relative aspect-video w-40 flex-shrink-0 bg-card overflow-hidden [@media(hover:hover)]:group-hover:outline [@media(hover:hover)]:group-hover:outline-[2px] [@media(hover:hover)]:group-hover:outline-white [@media(hover:hover)]:group-hover:outline-offset-2 [@media(hover:hover)]:group-has-[.group\/menu:hover]:outline-0"
        >
          <img 
          ...
          {progress && progressPercentage > 0 && (
            <div className="absolute bottom-0 left-0 right-0 h-2.5 bg-gradient-to-t from-black/60 to-transparent z-10">
          ...
          <div className="absolute bottom-1.5 right-1.5 bg-cr-black/90 px-1.5 py-0.5 rounded text-xs text-foreground font-semibold z-10">
        ...
        <div className="flex-1 space-y-1 pt-1">
          <div className="flex items-start justify-between gap-2">
            ...
            {/* Three dots menu - aligned with episode title */}
            <div className="flex-shrink-0">
              <DropdownMenu>
                <DropdownMenuTrigger asChild onClick={(e) => e.stopPropagation()}>
                  ...
                  >
                    <MoreVertical className="w-4 h-4 break-800:w-5 break-800:h-5 text-muted-foreground [@media(hover:hover)]:group-hover/menu:text-primary transition-colors" />
                  </button>
                </DropdownMenuTrigger>
